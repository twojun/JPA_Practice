...
(5) Cascade 전파 옵션의 경우, 라이프 사이클에 대해서 다른 연관된 엔티티도 동일하게 관리할 수 있을 때 사용하는 것이 좋다.</br></br>

(6) 특정 객체를 생성해야 하는 경우 new를 통해 생성해서 setter()를 사용하지 말고 별도의 객체 생성 메서드를 별도로 만드는 방식으로
...
(2) 그러나 JPA를 사용하게 되면, 엔티티의 데이터만 변경해 줘도 JPA가 알아서 변경된 포인트들을 변경 감지를 통해 트랜잭션 커밋 시 데이터베이스에 관련된 쿼리를 모두 전송해준다. </br></br>
